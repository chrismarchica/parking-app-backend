...
def load_sample_data():
    """
    Load sample violation data for demonstration purposes.
    
    Returns:
    - JSON confirmation of data loading
    """
    try:
        sample_size = request.json.get('sample_size', 1000) if request.is_json else 1000
        
        if not (100 <= sample_size <= 10000):
            return jsonify({
                'error': 'Sample size must be between 100 and 10000'
            }), 400
...
@parking_bp.route('/debug/data-status')
def debug_data_status():
    """
    Debug endpoint to check the status of loaded data.
    
    Returns:
    - JSON object with data loading status and sample records
    """
    try:
        data_loader = current_app.data_loader
        
        # Check parking signs data
        signs_status = {
            'loaded': data_loader.parking_signs_df is not None,
            'count': len(data_loader.parking_signs_df) if data_loader.parking_signs_df is not None else 0,
            'columns': list(data_loader.parking_signs_df.columns) if data_loader.parking_signs_df is not None and not data_loader.parking_signs_df.empty else [],
            'sample': data_loader.parking_signs_df.iloc[0].to_dict() if data_loader.parking_signs_df is not None and not data_loader.parking_signs_df.empty else None
        }
        
        # Check meter zones data
        meters_status = {
            'loaded': data_loader.meter_zones_df is not None,
            'count': len(data_loader.meter_zones_df) if data_loader.meter_zones_df is not None else 0,
            'columns': list(data_loader.meter_zones_df.columns) if data_loader.meter_zones_df is not None and not data_loader.meter_zones_df.empty else [],
            'sample': data_loader.meter_zones_df.iloc[0].to_dict() if data_loader.meter_zones_df is not None and not data_loader.meter_zones_df.empty else None
        }
        ...
        url_tests = {}
        for name, url in [('parking_signs', Config.PARKING_SIGNS_URL), ('meter_zones', Config.METER_ZONES_URL)]:
            try:
        ...
        return jsonify({
            'parking_signs': signs_status,
            'meter_zones': meters_status,
            'url_tests': url_tests,
            'config': {
                'signs_url': Config.PARKING_SIGNS_URL,
                'meters_url': Config.METER_ZONES_URL,
                'max_records': 50000
            }
        })
